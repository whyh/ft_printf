...
int	ft_printf_l(va_list *args, t_printf_mods mods, t_printf_buff *buff)
{
	...
	if (ft_strin(PRINTF_NDEC, mods.conv) || ft_strin(PRINTF_USN, mods.conv))
		buff->buff = ft_itoabase_unsigned(base, va_arg(*args, UL));
	else if (ft_strin(PRINTF_FLOAT, mods.conv)
	|| ft_strin(PRINTF_SN, mods.conv))
		buff->buff = ft_itoabase(base, va_arg(*args, L));
	else
		return (0);
	return (1);
}
...
int	ft_printf_h(va_list *args, t_printf_mods mods, t_printf_buff *buff)
{
	...
	if (ft_strin(PRINTF_USN, mods.conv) || ft_strin(PRINTF_NDEC, mods.conv))
		buff->buff = ft_itoabase_unsigned(base, (US)va_arg(*args, int));
	else if (ft_strin(PRINTF_SN, mods.conv))
		buff->buff = ft_itoabase(base, (S)va_arg(*args, int));
	else
		return (0);
	return (1);
}
...
int	ft_printf_ll(va_list *args, t_printf_mods mods, t_printf_buff *buff)
{
	...
	if (ft_strin(PRINTF_NDEC, mods.conv) || ft_strin(PRINTF_USN, mods.conv))
		...
		buff->buff = ft_itoabase(base, va_arg(*args, LL));
	else
		return (0);
	return (1);
}
...
int	ft_printf_hh(va_list *args, t_printf_mods mods, t_printf_buff *buff)
{
	...
	base = ft_printf_base(mods.conv);
	if (ft_strin(PRINTF_NDEC, mods.conv) || ft_strin(PRINTF_USN, mods.conv))
		buff->buff = ft_itoabase_unsigned(base, (UC)va_arg(*args, int));
	else if (ft_strin(PRINTF_SN, mods.conv))
		buff->buff = ft_itoabase(base, (char)va_arg(*args, int));
	else
		return (0);
	return (1);
}
...
int	ft_printf_cap_l(va_list *args, t_printf_mods mods, t_printf_buff *buff)
{
	...
	if (ft_strin(PRINTF_FLOAT, mods.conv))
	{
		arg = va_arg(*args, LD);
		if (arg < 0 && (arg = -arg))
			buff->buff = ft_strdup("-");
